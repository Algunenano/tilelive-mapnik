...
MapnikSource.prototype._open = function(uri) {
    ...
    this._mapnik_metrics = mapnik.supports.metrics && uri.query.mapnik_metrics || false;
    this._stats = {
...
MapnikSource.prototype._createMetatileCache = function() {
    ...
    this._tileCache = new LockingCache(function(key) {
        ...
        var keys = source._renderMetatile({
            ...
            x: +coords[2],
            y: +coords[3],
            metrics: source._mapnik_metrics
        }, function(err, tiles, metrics) {
            if (err) {
                ...
                for (var key in tiles) {
                    cache.put(key, null, tiles[key].image, tiles[key].headers, metrics);
                }
            }
        });
...
MapnikSource.prototype.getTile = function(z, x, y, callback) {
    ...
    this._tileCache.get(key, callback);
};
...
MapnikSource.prototype.getGrid = function(z, x, y, callback) {
    ...
    this._tileCache.get(key, function(err, grid, headers, metrics) {
        if (err) return callback(err);
        delete grid.solid;
        callback(null, grid, headers, metrics);
    });
};
