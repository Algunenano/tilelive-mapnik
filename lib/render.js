...
function sliceMetatile(source, image, options, meta, callback) {
    var tiles = {};
    var metrics = image.get_metrics();
    if (options.metrics) {
        metrics["tiles in metatile"] = meta.tiles.length;
    }
    ...
    Step(function() {
        ...
        callback(null, tiles, metrics);
    });
}

exports['encodeSingleTile'] = encodeSingleTile;
function encodeSingleTile(source, source_image, options, meta, callback) {
    var tiles = {};
    var key = [options.format, options.z, options.x, options.y].join(',');
    getImage(source, source_image, options, 0, 0, function(err, image) {
        if (err) return callback(err);
        var metrics = source_image.get_metrics();
        if (options.metrics) {
            metrics["tiles in metatile"] = 1;
        }
